...
export const summarizeModes = [
  { key: 'summary', label: '重點整理', prompt: '錄音文字是一段可能由多人或單人錄製, 由 OPENAI 處理聲音轉文字的逐字稿, 專有名詞上可能會有錯誤, 或每次音譯造成不同, 而且可能各國廣告或是歡迎訂閱請刪除. 請參考使用者補充筆記將這段文字整理成清楚條列式的重點摘要。' },
  { key: 'analysis', label: '會議記錄', prompt: '錄音文字是一段可能由多人或單人錄製, 由 OPENAI 處理聲音轉文字的逐字稿, 專有名詞上可能會有錯誤, 或每次音譯造成不同, 而且可能各國廣告或是歡迎訂閱請刪除. 請參考使用者補充筆記將這段文字整理成會議記錄, 包含參與者(如果有提及), 會議時間(如果有提及), 討論項目, 下一步行動(依照日期排列)。' },
  { key: 'email', label: '信件撰寫', prompt: '錄音文字是一段可能由多人或單人錄製, 由 OPENAI 處理聲音轉文字的逐字稿, 專有名詞上可能會有錯誤, 或每次音譯造成不同, 而且可能各國廣告或是歡迎訂閱請刪除. 請參考使用者補充筆記幫我把這段文字整理成一封正式的商業郵件，語氣禮貌。' },
  { key: 'news', label: '新聞稿', prompt: '錄音文字是一段可能由多人或單人錄製, 由 OPENAI 處理聲音轉文字的逐字稿, 專有名詞上可能會有錯誤, 或每次音譯造成不同, 而且可能各國廣告或是歡迎訂閱請刪除. 請參考使用者補充筆記將這段文字改寫成新聞稿格式，具體且吸引人。' },
  { key: 'ai_answer', label: 'AI給答案', prompt: '錄音文字是一段可能由多人或單人錄製, 由 OPENAI 處理聲音轉文字的逐字稿, 專有名詞上可能會有錯誤, 或每次音譯造成不同, 而且可能各國廣告或是歡迎訂閱請刪除. 請參考使用者補充筆記將這段文字，直接給出一個完整詳細的回答。' },
];
